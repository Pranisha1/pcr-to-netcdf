...
#This python file to convert from pcr to netcdf
...
# Get the list of files inside the zip archive using unzip -Z1
file_list=$(unzip -Z1 "$zip_filename") #Using -Z1 sets the mode to Zipinfo and 1 allows the Zipinfo to displat the names of the files, one per line
...
for filename in $file_list; do
    ...
    unzip -j "$zip_filename" "$filename" -d "$tmproot"  #-j "junk paths" will extract only the file without the zipfolder; -d "$tmproot" specifies where to store the files
    ...
    if [ -f "$input_file" ]; then
        ...
        #output_file="${outroot}/${filename%.*}_r.nc"
        ...
        if [ $? -eq 0 ]; then
            ...
            # Check for NaN and -999999 values in the output file
            if cdo info "$output_file" | grep -Eq "nan|-999999"; then
                echo -e "\e[1;31mWarning: NaN or -999999 values detected in $output_file\e[0m"
            else
                echo -e "\e[1;36mNo NaN or -999999 values detected in $output_file\e[0m"
            fi	
            ...
            rm "$input_file"
            echo -e "\e[1;35mProcessed, moved, and deleted: $input_file -> $output_file"
        else
...
echo -e "\e[34;43mNOW COMBINE THE AND ZIP THE FILES.\e[0m"
...
rm -r "$tmproot"
rm -r "$remapped_tmpdir"

#Using the function ncdump to display the structure of the NetCDF file:
